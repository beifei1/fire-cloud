...
//    triggers { pollSCM('H/1 0 * * *')}
    ...
    stages {
        ...
        stage ("构建安装") {
            ...
            steps {
               configFileProvider([configFile(fileId: 'd4231502-faae-45f4-b0d9-c4bff6e15692',targetLocation: 'setting.xml', variable: 'MAVEN_GLOBALE_SETTING')]) {
                   sh "mvn -f ${params.pom_path} -s $MAVEN_GLOBALE_SETTING -U install -Dmaven.skip.test=true"
               }
            }
        }
        ...
        stage ("构建发布") {
            ...
            steps {
               configFileProvider([configFile(fileId: 'd4231502-faae-45f4-b0d9-c4bff6e15692',targetLocation: 'setting.xml', variable: 'MAVEN_GLOBALE_SETTING')]) {
                   sh "mvn -f ${params.project_name}/pom.xml -s -U $MAVEN_GLOBALE_SETTING deploy -Dmaven.skip.test=true"
               }
            }
        }
    ...
    post {
        ...
        success {
            emailext  body: '''${FILE, path="/var/lib/jenkins/notify.html"}''',
            mimeType: 'text/html',
            subject: "[Jenkins Pipeline][${JOB_NAME}]构建成功: - #${BUILD_NUMBER} Success!",
            to: "${_build_state_notify_to}",
            from: "${_build_state_notify_from}"
        }
    }
